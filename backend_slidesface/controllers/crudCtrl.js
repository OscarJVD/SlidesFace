 ...
 */
  createField: async (req, res) => {
    try {
      const { model, forallusersflag, newFields, fields, modelRef, subModelFlag } = req.body;
      let { values } = req.body;

      if (subModelFlag) {
        let subModelNewValue = { '0': values[0] }
        subModelNewValue[values[0].inputAndModelName] = values[0].value;
        values = subModelNewValue
      }
      ...
      if (models.includes(model)) {
        ...
        // No se ha compilado - creado el modelo
        ...
        if (newFieldsFlag) {
          newFields.forEach(newField => {
            ...
            if (newField.type == 'text' || newField.type == 'string' || newField.type == '') {
              type = `String`
            }

            if (newField.type == 'number' || newField.type == 0) {
              type = `Number`
            }
          ...
          if (model == 'pruebako') {
            console.log('prueba SubModel')
            console.log(util.inspect(fields));
          }

          for (const keyField in fields) {
            schemaObj += `${keyField}: {
              type: ${typeof fields[keyField] == 'string' ? `String` : `Number`},
              required: true
            },`
          }
        }
...
*/
  getDataField: async (req, res) => {
    ...
    try {
      ...
      const { model, forallusersflag, otherUser } = req.body;
...
*/
  softDeleteRow: async (req, res) => {
    try {
      ...
      mongoose.connection.db.listCollections({ name: model + 'historic' + 's' })
